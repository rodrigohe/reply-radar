...
import { colorClasses } from "@/app/lib/utils";
...
export default function LocationComboBox({
  ...
  defaultLocations: LocationColors[];
  readOnly?: boolean;
}) {
  ...
  const selectedSet = new Set(selectedLocations.map(l => l.name));
  const defaultSet = new Set(defaultLocations.map(l => l.name));
  const newLocations = selectedSet.difference(defaultSet);
  const newLocationsColors = selectedLocations.filter(l => newLocations.has(l.name));
  ...
  return (
    <div>
      ...
      <input type="hidden" name="new_location" value={newLocations.size > 0 ? [...newLocations].join(', ') : ''}></input>
      <input type="hidden" name="new_location_colors" value={newLocationsColors.length > 0 ? JSON.stringify(newLocationsColors) : ''}></input>
    </div>
  );
}
